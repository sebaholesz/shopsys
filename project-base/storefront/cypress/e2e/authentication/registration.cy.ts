...
import { checkAndHideSuccessToast, checkUrl, goToEditProfileFromHeader, takeSnapshotAndCompare } from 'support';
...
describe('Registration tests', () => {
    ...
    it('should disallow registration with invalid info, but then allow after correction', () => {
        ...
        cy.getByTID([TIDs.registration_submit_button]).click();
        cy.getByTID([TIDs.toast_error]).contains('This email is already registered').should('be.visible');
    });
});
